...
def _find_tsc_value_in_df(df, material_no, row_label):
    ...
    _, spec_row, comp_col = _resolve_tsc_raw_layout(df)
...
def _find_tsc_metrics_in_df(df, material_no, row_label):
    ...
    key_cols = _resolve_tsc_key_columns(df)
    _, spec_row, _ = _resolve_tsc_raw_layout(df)
    mat_col = field_cols.get('修行后原料', key_cols.get('mat_col', 2))
    label_col = field_cols.get('行类型', key_cols.get('label_col', 4))
    start_row = max(
        (max(header_rows) + 1) if header_rows else 0,
...
def _find_tsc_reference_metrics(tsc_file, sheet_name, material_no, reference_label, quarter_label):
    try:
        df = pd.read_excel(_as_bytes_io(tsc_file), sheet_name=sheet_name, header=None)
    except Exception:
        return None
    return _find_tsc_reference_metrics_in_df(df, material_no, reference_label, quarter_label)


def _find_tsc_reference_metrics_in_df(df, material_no, reference_label, quarter_label):
...
def _find_tsc_reference_value(tsc_file, sheet_name, material_no, reference_label, quarter_label):
    try:
        df = pd.read_excel(_as_bytes_io(tsc_file), sheet_name=sheet_name, header=None)
    except Exception:
        return None
    return _find_tsc_reference_value_in_df(df, material_no, reference_label, quarter_label)


def _find_tsc_reference_value_in_df(df, material_no, reference_label, quarter_label):
...
def _find_tsc_row_values(df, material_no, row_label, raw_cols, comp_col):
    ...
    start_row = max(key_cols.get('key_row', 0) + 1, (spec_row + 1) if spec_row is not None else 0)
    for i in range(start_row, len(df)):
        mat = _normalize_mat(df.iat[i, mat_col]) if mat_col < df.shape[1] else ''
        label = str(df.iat[i, label_col]).strip() if label_col < df.shape[1] else ''
        if mat == material_no and _match_tsc_row_label(label, row_label):
...
def _find_tsc_reference_row_values(df, material_no, reference_label, quarter_label, raw_cols, comp_col):
    for label in _quarter_actual_labels(quarter_label):
...
def _find_tsc_reference_ratio_row_values(df, material_no, reference_label, quarter_label, raw_cols, comp_col):
    for label in _quarter_ratio_labels(quarter_label):
        values = _find_tsc_row_values(df, material_no, label, raw_cols, comp_col)
        if values:
            return values
...
def compute(compare_file, rawlist_file, q3_file, map_file, sheet_name=None, month_label='11月', quarter_label='Q3', month_code=''):
    ...
    for mat, v in agg.items():
        ...
        q3_ratio = _find_tsc_reference_value_in_df(q3_df, mat, reference_ratio_label, quarter_label)
        nov_ratio = None
        q3_metrics = _find_tsc_reference_metrics_in_df(q3_df, mat, reference_price_label, quarter_label)
    ...
    def build_matrix_rows(materials, raw_cols, comp_col, df_q3, category_label):
        ...
        for mat in materials:
            ...
            q3_price = _find_tsc_reference_row_values(df_q3, mat, reference_price_label, quarter_label, raw_cols, comp_col)
            q3_ratio = _find_tsc_reference_ratio_row_values(df_q3, mat, reference_ratio_label, quarter_label, raw_cols, comp_col)
